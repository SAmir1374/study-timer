...
import { state, commit, rebuildSummaries, getNextSessionPractice } from "./state.js";
import {
  activeMs,
  createSessionRecord,
  finalizeRecord,
...
export const TimerEngine = {
  ...
  /**
   * subject and isPractice are only used for "study" sessions (breaks are
   * always subject:null / isPractice:false).
   * When omitted, the currently selected values are used
   * (settings.lastSelectedSubject / settings.lastSelectedPractice) —
   * pass them explicitly only when overriding those defaults.
   */
  start(durationMs, type, subject, isPractice) {
    const s = state.session;
    const now = Date.now();
    ...
    if (s.state === "idle" || s.state === "complete") {
      ...
      const sub =
        kind === "study" ? (subject !== undefined ? subject : state.doc.settings.lastSelectedSubject) : null;
      const practice =
        kind === "study" ? (isPractice !== undefined ? !!isPractice : getNextSessionPractice()) : false;
  ...
  _toIdle() {
    ...
    s.isPractice = getNextSessionPractice();
