...
/** subject: a string from doc.subjects, or null for "no subject". Only affects the next study session. */
...
export function addSubject(name) {
  ...
  mutateDocument('subjects', (doc) => {
    if (!doc.subjects.includes(trimmed)) {
...
export function removeSubject(name) {
  mutateDocument('subjects', (doc) => {
    doc.subjects = doc.subjects.filter((s) => s !== name);
...
/** Today's study time broken down by subject, sorted by time desc. Sessions with no subject are skipped. */
